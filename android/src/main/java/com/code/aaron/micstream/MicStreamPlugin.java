...
public class MicStreamPlugin implements FlutterPlugin, EventChannel.StreamHandler, MethodCallHandler {
  ...
  byte[] buffer1000ms = new byte[16000];
  byte[] floatBuffer1000ms = new byte[16000];
  byte[] floatTemp = new byte[buffer1000ms.length];
    ...
    public void run() {
      ...
      while (record) {
        ...
        try {
          ...
          System.arraycopy(data,0,floatTemp,floatTemp.length-data.length,data.length);
//                System.arraycopy(buffer1000ms,buffer.length,temp,0,buffer1000ms.length-buffer.length);
          System.arraycopy(floatBuffer1000ms,data.length,floatTemp,0,floatBuffer1000ms.length-data.length);
